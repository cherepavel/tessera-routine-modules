...
public class DataEnrichmentRoutineJarIT {
    ...
    public void shouldExposeMetadataFromPackagedJar() throws Exception {
        ...
        Path modulesDirectory = Files.createTempDirectory("data-enrichment-routine-it-");
        ...
        try (var loader = RoutineEditorMetadataLoaderFactory.create(
            ...
            var metadata = loader.findMetadata("DataEnrichmentWorkflowRoutine").orElseThrow();

            assertEquals("1.0.0", metadata.getVersion());
            assertEquals(isolatedArtifact.getFileName().toString(), metadata.getArtifactFileName());
            assertEquals(3, metadata.getDescriptor().getFunctions().size());
        } finally {
