...
import io.github.byzatic.tessera.industrial_pipeline.workflowroutine.get_data.application.GetDataFunctionHandler;
import io.github.byzatic.tessera.industrial_pipeline.workflowroutine.get_data.infrastructure.generators.ConfiguredMetricGeneratorProvider;
...
public class GetDataWorkflowRoutine extends AbstractWorkflowRoutine {
    private final static Logger logger = LoggerFactory.getLogger(GetDataWorkflowRoutine.class);
    private final RunRoutineUseCase runRoutine;

    public GetDataWorkflowRoutine(MCg3WorkflowRoutineApiInterface workflowRoutineApi, HealthFlagProxy healthFlagProxy) throws MCg3ApiOperationIncompleteException {
        super(
                GetDataWorkflowRoutine.class,
                workflowRoutineApi,
        ...
        try {
            this.runRoutine = TesseraRoutineApplicationFactory.create(
                    workflowRoutineApi,
                    new GetDataFunctionHandler(
                            ConfiguredMetricGeneratorProvider.from(workflowRoutineApi)
                    )
            );
        } catch (Exception e) {
